...
def get_potus_model(exported_stan_data):

    data = json.load(open(exported_stan_data))
    ...
    with pm.Model() as m:
        ...
        raw_polling_bias = pm.Normal(
            "raw_polling_bias", mu=0.0, sigma=1.0, shape=(np_data["S"])
        )

        raw_mu_b_T = pm.Normal("raw_mu_b_T", mu=0.0, sigma=1.0, shape=(np_data["S"]))
        raw_mu_b = pm.Normal(
            "raw_mu_b", mu=0.0, sigma=1.0, shape=(np_data["S"], np_data["T"])
        )
        raw_mu_c = pm.Normal("raw_mu_c", mu=0.0, sigma=1.0, shape=(np_data["P"]))
        raw_mu_m = pm.Normal("raw_mu_m", mu=0.0, sigma=1.0, shape=(np_data["M"]))
        raw_mu_pop = pm.Normal("raw_mu_pop", mu=0.0, sigma=1.0, shape=(np_data["Pop"]))
        ...
        raw_e_bias = pm.Normal("raw_e_bias", mu=0.0, sigma=1.0, shape=(np_data["T"]))
        raw_measure_noise_national = pm.Normal(
            "raw_measure_noise_national",
            mu=0.0,
            sigma=1.0,
            shape=(np_data["N_national_polls"]),
        )
        raw_measure_noise_state = pm.Normal(
            "raw_measure_noise_state",
            mu=0.0,
            sigma=1.0,
            shape=(np_data["N_state_polls"]),
        )

        polling_bias = pm.Deterministic(
            "polling_bias",
            theano.tensor.dot(cholesky_ss_cov_poll_bias, raw_polling_bias),
        )
        ...
        # Reverse these (?)
        innovs = theano.tensor.transpose(theano.tensor.transpose(innovs)[::-1])

        # Tack on the "first" one:
        together = pm.math.concatenate(
            [theano.tensor.reshape(mu_b_final, (-1, 1)), innovs], axis=1
        )

        # Compute the cumulative sums:
        cumsums = theano.tensor.cumsum(together, axis=1)

        # To be [time, states]
        transposed = theano.tensor.transpose(cumsums)

        mu_b = pm.Deterministic("mu_b", theano.tensor.transpose(transposed[::-1]))

        national_mu_b_average = pm.Deterministic(
            "national_mu_b_average",
            pm.math.matrix_dot(
                theano.tensor.transpose(mu_b), np_data["state_weights"].reshape((-1, 1))
            ),
        )[:, 0]
        ...
        sigma_vec = pm.math.concatenate(
            [
                ...
                theano.tensor.repeat(sigma_rho, np_data["T"] - 1),
            ]
        )
        mus = pm.math.concatenate(
            [
                [0.0],
                theano.tensor.repeat(mu_e_bias * (1 - rho_e_bias), np_data["T"] - 1),
            ]
        )
        ...
        prob_state = theano.tensor.nnet.sigmoid(logit_pi_democrat_state)
        prob_nat = theano.tensor.nnet.sigmoid(logit_pi_democrat_national)
